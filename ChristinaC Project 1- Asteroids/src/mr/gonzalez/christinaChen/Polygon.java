...
class Polygon {
  ...
  public Point pos;   // The offset mentioned above.
  public double rot; // Zero degrees is due east.
...
//    for (Point p : shape) {
      Point p = shape[i];
      double x = ((p.x-center.x) * Math.cos(Math.toRadians(rot)))
               - ((p.y-center.y) * Math.sin(Math.toRadians(rot)))
               + center.x/2 + pos.x;
      double y = ((p.x-center.x) * Math.sin(Math.toRadians(rot)))
               + ((p.y-center.y) * Math.cos(Math.toRadians(rot)))
               + center.y/2 + pos.y;
  ...
  public void rotate(int degrees) {rot = (rot+degrees)%360;}
  ...
  */
  ...
  private Point findCenter() {
	...
	* Use contain method to check for Polygon intersection
	*
